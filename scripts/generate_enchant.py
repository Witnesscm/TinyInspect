...
from wago_tools import fetch_csv, fetch_version
...
def main():
    ...
    # Spell ID -> Spell Name
    spell_name_csv = fetch_csv("SpellName", PRODUCT, "zhCN")
    spell_name = {int(r["ID"]): r["Name_lang"] for r in spell_name_csv}

    # Item ID -> Item Name
    item_sparse_csv = fetch_csv("ItemSparse", PRODUCT, "zhCN")
    ...
    # Effect ID -> Spell ID
    item_effect_csv = fetch_csv("ItemEffect", PRODUCT)
    ...
    item_x_effect_csv = fetch_csv("ItemXItemEffect", PRODUCT)
    ...
    # SpellEffect
    crafting_to_spell = {}
    spell_effect_csv = fetch_csv("SpellEffect", PRODUCT)
    ...
    crafting_data_csv = fetch_csv("CraftingDataEnchantQuality", PRODUCT)
    ...
    item_csv = fetch_csv("Item", PRODUCT)
    ...
    output_file = os.path.join(ROOT_PATH, "ItemEnchant.lua")
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("-- Generated by script from https://wago.tools/db2\n")
        f.write(f"-- Last Updated: {version}\n")
